...
supervisor_has_send_path_for_agent() {
  ...
  local patterns=(
    "agent:${agent}:"
    "${agent}.*sessions_send|sessions_send.*${agent}"
    "${agent}.*sendStatus|sendStatus.*${agent}"
...
for raw in "${REQUIRED_LIST[@]}"; do
  ...
  if [[ "$source_kind" == "missing" ]]; then
    if supervisor_has_send_path_for_agent "$agent"; then
      echo "WARN: ${agent} 缺少 worker 侧任务证据，但 supervisor 已出现 send-path 轨迹"
      send_path_only+=("$agent")
    else
