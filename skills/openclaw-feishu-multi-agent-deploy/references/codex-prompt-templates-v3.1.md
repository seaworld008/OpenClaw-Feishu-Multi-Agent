# 飞书多 Agent 主管派单与自动跨群收口交付蓝图（V3.1）
...
- V3.1：在 V3 基础上补齐“真实派单硬门禁 + sandbox 会话可见性 + 可审计验收”，用于正式交付。

## V3.1 相比 V3 新增的关键约束

1. 不再接受“文本模拟派单”。
- 未完成 `sessions_list` + `3 次 sessions_send` + `dispatchEvidence`，一律判定失败。

2. 明确处理 sandbox 会话可见性。
- 若 `supervisor_agent` 运行在 sandbox 中，仅设置 `tools.sessions.visibility="all"` 还不够；还要检查 `sessionToolsVisibility` 是否允许看到非树状会话。

3. 验收从“看回复”升级为“看证据”。
- 必须同时验证：主管回复、三目标会话轨迹、dispatchEvidence、canary 脚本结果。
...
7. 若 `supervisor_agent` 使用 sandbox：
- 还需检查 `agents.defaults.sandbox.sessionToolsVisibility` 或 `supervisor_agent.sandbox.sessionToolsVisibility`
- 建议：`"all"`，否则跨群会话可见性可能被限制
...
首次启用 V3.1 前，必须先让每个业务群至少触发一次机器人（例如发送 `@机器人 /status`），让目标会话存在。否则主管派单可能找不到目标 sessionKey。

## 一次性交付主提示词（V3.1，可直接发 Codex）

```text
请使用 openclaw-feishu-multi-agent-deploy skill，按官方最新规范完成 V3.1 交付：
实现“自动跨群收口 + 主管派单 -> 三群执行 -> 主管自动收口”，并确保可审计、可验收、不可文本伪装。
...
- 若存在 sandbox 限制，最小化放开会话工具可见性，确保 supervisor 能看到三业务群会话。
...
- { id: "supervisor_agent", role: "主管派单与收口", systemPrompt: "你是主管 Agent。唯一正确流程：1) 先调用 sessions_list 找到 sales/ops/finance 三个目标会话；2) 为每个会话生成独立任务卡并执行 3 次 sessions_send；3) 等待结果或记录超时/失败；4) 只有在 dispatchEvidence 完整时才输出正式收口稿。硬约束：禁止文本模拟派单；禁止跳过 sessions_list；禁止在无证据时声称已派发。若任一步未完成，首行必须返回 DISPATCH_INCOMPLETE。成功输出时必须包含 dispatchEvidence 数组，每条至少有 agentId、sessionKey、taskId、sendStatus、sentAt、evidenceSource。" }
...
2. 只允许改：channels.feishu、bindings、agents.list、tools.allow、tools.agentToAgent、tools.sessions、session.sendPolicy，以及为 supervisor 必要时最小修改的 sandbox 会话可见性字段。
...
13. 如果发现 sandbox 限制导致 `tools.sessions.visibility="all"` 仍无法看到三业务群会话，必须显式修正 `sessionToolsVisibility` 后再交付。
14. `dispatchEvidence` 不能只写自然语言摘要，必须是结构化列表。
15. 若 `sessions_send` 返回 `ok` 但无法证明最终送达，必须标记为 `accepted_not_fully_verified`，不得写成“已完成派单”。
...
3. 将上面的 V3.1 主提示词发给 Codex 生成 patch。  
4. 先备份配置，再应用 patch。  
5. 执行校验和重启：
- `openclaw config validate`
- `openclaw gateway restart`
- `openclaw agents list --bindings`
6. 如果启用了 sandbox，额外检查 `sessionToolsVisibility` 是否允许跨群会话可见。  
7. 在主管群做 canary 演示（见下节）。  
8. 执行 canary 自动校验（见下方命令），检查 2 分钟窗口内是否出现三条会话派发证据。  
9. 若返回 `DISPATCH_UNVERIFIED`，补查原始日志中的 `sessions_send` 证据。  
10. 验收通过后再全量使用。

### V3.1 canary 自动校验命令（2 分钟窗口）
