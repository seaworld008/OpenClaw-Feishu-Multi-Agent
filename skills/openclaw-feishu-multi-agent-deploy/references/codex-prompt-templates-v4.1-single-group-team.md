...
> 状态：过渡版本。若你要在当前 OpenClaw + Feishu 环境里做真实单群交付，优先改用 `V4.2`；`V4.1` 仅保留作受控互审设计参考。
...
2. 把 3 个机器人都拉入同一个群。  
3. 在群里依次做 warm-up：  
- `@小龙虾找妈妈 WARMUP team-v4-1-001-ops`
- `@易燃易爆 WARMUP team-v4-1-001-fin`
- `@奥特曼 /status`
...
1. `@小龙虾找妈妈 WARMUP team-v4-1-001-ops`
2. `@易燃易爆 WARMUP team-v4-1-001-fin`
